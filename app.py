...
def get_ts(item, attr):
    return datetime.utcfromtimestamp(min(int(item.get(attr)) / 1000, 253401772800))
...
def get_prob_resolved(market):
    if market.get("resolution") == "NO":
        ...
        try:
            ...
            print(
                "WARN: resolutionProbability does not exist on market",
                market["id"],
                "- using 0 instead.",
            )
            return 0
...
def get_prob_at_pct(bets, percent):
    timestamp = (
        bets[0]["timestamp"]
        + (bets[len(bets) - 1]["timestamp"] - bets[0]["timestamp"]) * percent
    )
    for bet in bets:
...
def get_prob_time_weighted(bets):
    ...
    for bet in bets[1:]:
        prob_weighted += (
            bet["prob_before"]
            * (
                bet["timestamp"] - bets[bets.index(bet) - 1]["timestamp"]
            ).total_seconds()
        )
    return (
        prob_weighted
        / (bets[len(bets) - 1]["timestamp"] - bets[0]["timestamp"]).total_seconds()
    )
...
def get_payout(market, bets):
    ...
    if market["resolution"] in positions.keys():
        ...
        return positions["YES"] * market["resolutionProbability"] + positions["NO"] * (
            1 - market["resolutionProbability"]
        )
...
def refresh_data():
    ...
    cached_ids = [
        market["manifold_id"] for market in Market.select(Market.manifold_id).dicts()
    ]
    ...
    for market in markets_raw:
        if not markets_raw.index(market) % 5000:
            ...
            print(
                "Data download:",
                markets_raw.index(market),
                "/",
                len(markets_raw),
                "@",
                round(markets_raw.index(market) / (datetime.now() - ts0).seconds),
                "mps",
            )
        if (
            ...
            and not market.get("resolution") == "CANCEL"
            and not market["id"] in cached_ids
        ):
            ...
            save_market(
                {
                    ...
                    "prob_time_weighted": get_prob_time_weighted(
                        clean_bets(market, bets)
                    ),
                }
            )
    ...
    if request.form.get(attr + "_val") and request.form.get(attr + "_mod"):
        if request.form.get(attr + "_mod") == "gt":
            markets = markets.where(
                getattr(Market, attr) >= request.form.get(attr + "_val")
            )
...
def get_data():
    ...
    xbins = {}
    if request.form.get("xbin_size") and round(
        float(request.form.get("xbin_size")), 3
    ) in [round(i, 3) for i in np.arange(0.005, 1, 0.005)]:
    ...
    for market in markets.dicts().iterator():
        ...
        xb = round(
            int((float(market[xaxis_attr]) - xbin_size / 2) / xbin_size) * xbin_size
            + xbin_size / 2,
            4,
        )
        if not xb in xbins.keys():
            print(
                "Error: Count not fit market",
                market["manifold_id"],
                xaxis_attr,
                market[xaxis_attr],
                "->",
    ...
    for xv in xbins:
        ...
        if len(xb["resolved"]):
            ...
            sumproduct = sum(
                [
                    xb["resolved"][i] * xb["yaxis_weight"][i]
                    for i in range(len(xb["resolved"]))
                ]
            )
            ...
            brier_cumsum += sum(
                [
                    (xb["resolved"][i] - xb["forecast"][i]) ** 2 * xb["yaxis_weight"][i]
                    for i in range(len(xb["resolved"]))
                ]
            )
            brier_weight += sum(
                [xb["yaxis_weight"][i] for i in range(len(xb["resolved"]))]
            )
            ...
            if point_attr == "none":
                ...
                sum_attr = sum(
                    [xb["point_weight"][i] for i in range(len(xb["resolved"]))]
                )
                data["point_size"].append(sum_attr)
                data["point_desc"].append(
                    point_data[point_attr]["prefix"]
                    + str(sum_attr)
                    + point_data[point_attr]["postfix"]
                )
